...
macro_rules! my_helper {
    ...
    (
        [fields: $($fields:tt)*]
        [fns_impl: $($fns_impl:tt)*]
        [fns_no_impl: $($fns_no_impl:tt)*]
        let $var_name:ident : $var_type:ty;
        $($rest:tt)*
    ) => (my_helper! {
        [fields: $($fields)* $var_name $var_type]
        [fns_impl: $($fns_impl)*]
        [fns_no_impl: $($fns_no_impl)*]
        $($rest)*
    });
    ...
    (
        ...
        [fields: $($fields)*]
        [fns_impl: $($fns_impl)* $fn_name $($arg)* $($ret_ty)? $($fn_body)*]
        [fns_no_impl: $($fns_no_impl)*]
        $($rest)*
    });
    ...
    (
        ...
        [fns_impl: $($fns_impl:tt)*]
        [fns_no_impl: $($fns_no_impl:tt)*]
        fn $fn_name:ident($($arg:tt)*) $(-> $ret_ty:ty)? ;
        $($rest:tt)*
    ) => (my_helper! {
        [fields: $($fields)*]
        [fns_impl: $($fns_impl)*]
        [fns_no_impl: $($fns_no_impl)* $fn_name $($arg)* $($ret_ty)?]
        $($rest)*
    });
    // 4.final output
    (
        [fields: $( $var_name:ident : $var_type:ty )*]
        [fns_impl: $( $fn_name_impl:tt $(arg_impl:tt)* $(ret_ty_impl:tt)? $fn_body:tt )*]
        [fns_no_impl: $( $fn_name_no_impl:tt $(arg_no_impl:tt)* $(ret_ty_no_impl:tt)? )*]
    ) => (
